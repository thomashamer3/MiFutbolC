...
static void mostrar_por_dia_semana(const char *titulo, const char *columna, const char *order_by,
                                   int limit)
{
    clear_screen();
    print_header(titulo);
    ...
    snprintf(sql, sizeof(sql),
             ...
             "ORDER BY %s%s%s",
             columna, order_by_columna ? "" : "promedio ", order_by, limit_clause);
...
void mostrar_estadisticas_generales(void)
{
    ...
    StatQuery queries[] =
    {
        {"Camiseta con mas Goles", SQL_CAMISETA_AGREGADA("IFNULL(SUM(p.goles),0)", "DESC")},
        {
...
{
    mostrar_query_simple("PROMEDIO DE GOLES POR PARTIDO", "Promedio de Goles por Partido",
...
{
    mostrar_query_simple("PROMEDIO DE RENDIMIENTO_GENERAL", "Promedio de Rendimiento General",
...
void mostrar_rendimiento_promedio_por_clima(void)
{
    ...
    mostrar_query_simple("RENDIMIENTO PROMEDIO POR CLIMA", "Rendimiento Promedio por Clima", sql);
}
...
void mostrar_clima_mejor_rendimiento(void)
{
    ...
    mostrar_query_simple("CLIMA DONDE SE RINDE MEJOR", "Clima con Mejor Rendimiento Promedio", sql);
}
...
void mostrar_clima_peor_rendimiento(void)
{
    ...
    mostrar_query_simple("CLIMA DONDE SE RINDE PEOR", "Clima con Peor Rendimiento Promedio", sql);
}
...
void mostrar_rendimiento_por_nivel_cansancio(void)
{
    ...
    mostrar_query_simple("RENDIMIENTO POR NIVEL DE CANSANCIO", "Rendimiento por Nivel de Cansancio",
                         sql);
}
...
{
    mostrar_query_simple("PARTIDOS JUGADOS CON CANSANCIO ALTO", "Partidos con Cansancio Alto (>7)",
...
void mostrar_caida_rendimiento_cansancio_acumulado(void)
{
    ...
    sqlite3_stmt *stmt;
    const char *sql = "SELECT 'Recientes (ultimos 5)' AS periodo, "
...
void mostrar_rendimiento_por_estado_animo(void)
{
    ...
    mostrar_query_simple("RENDIMIENTO POR ESTADO DE ANIMO", "Rendimiento por Estado de Animo", sql);
}
...
void mostrar_goles_por_estado_animo(void)
{
    ...
    mostrar_query_simple("GOLES POR ESTADO DE ANIMO", "Goles por Estado de Animo", sql);
}
...
void mostrar_asistencias_por_estado_animo(void)
{
    ...
    mostrar_query_simple("ASISTENCIAS POR ESTADO DE ANIMO", "Asistencias por Estado de Animo", sql);
}
...
void mostrar_estado_animo_ideal(void)
{
    ...
    mostrar_query_simple("ESTADO DE ANIMO IDEAL PARA JUGAR", "Estado de Animo Ideal", sql);
}
