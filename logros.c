...
typedef struct
{
    ...
    int objetivo;
    const char *tipo; // tipos disponibles: "goles", "asistencias", "partidos", "goles+asistencias",
    // "victorias", "empates", "derrotas", "rendimiento_general", "estado_animo",
    // "canchas_distintas", "hat_tricks", "poker_asistencias",
    // "rendimiento_perfecto", "animo_perfecto", "goles_victorias",
    // "asistencias_victorias", "rendimiento_victorias", "animo_victorias",
    // "goles_derrotas", "asistencias_derrotas", "rendimiento_empates",
    // "animo_empates", y muchos mas (ver LOGRO_QUERIES)
} Logro;
...
#define SQL_RACHA_MAX(resultado)                                                                   \
    "SELECT COUNT(*) FROM (SELECT resultado, ROW_NUMBER() OVER (ORDER BY id) - ROW_NUMBER() OVER " \
    "(PARTITION BY resultado ORDER BY id) as grp" SQL_FROM_PARTIDO                                 \
    ") WHERE resultado = " #resultado " GROUP BY grp ORDER BY COUNT(*) DESC LIMIT 1"

#define LOGRO_QUERY(tipo, sql) {tipo, sql}

static const LogroQuery LOGRO_QUERIES[] =
{
    LOGRO_QUERY("goles", SQL_SUM("goles")), LOGRO_QUERY("asistencias", SQL_SUM("asistencias")),
    ...
    LOGRO_QUERY("partidos_con_rendimiento_perfecto_y_animo",
                SQL_COUNT_WHERE("rendimiento_general = 10 AND estado_animo = 10")),
...
#define LOGRO_EXPERTO(nombre_base, descripcion, objetivo, tipo)                                    \
    LOGRO_ITEM(nombre_base " Experto", descripcion, objetivo, tipo)
#define LOGRO_EXPERTA(nombre_base, descripcion, objetivo, tipo)                                    \
    LOGRO_ITEM(nombre_base " Experta", descripcion, objetivo, tipo)
#define LOGRO_PAR(nombre1, desc1, obj1, nombre2, desc2, obj2, tipo)                                \
    LOGRO_ITEM(nombre1, desc1, obj1, tipo), LOGRO_ITEM(nombre2, desc2, obj2, tipo)
#define LOGRO_RACHAS(sufijo, objetivo)                                                             \
    LOGRO_ITEM("Racha de Victorias" sufijo, "Ganar " #objetivo " partidos consecutivos", objetivo, \
               "victorias_consecutivas_max"),                                                      \
        LOGRO_ITEM("Racha de Derrotas" sufijo, "Perder " #objetivo " partidos consecutivos",       \
                   objetivo, "derrotas_consecutivas_max"),                                         \
        LOGRO_ITEM("Racha de Empates" sufijo, "Empatar " #objetivo " partidos consecutivos",       \
                   objetivo, "empates_consecutivos_max")
...
static const Logro LOGROS[] =
{
    ...
    LOGRO_SERIE_NIVELES_5("Contribuidor", "Acumular", "puntos (goles + asistencias)",
                          "goles+asistencias", 10, 25, 50, 100, 250),
    // Victories
    ...
    // General Performance
    LOGRO_ITEM("Rendimiento Inicial", "Acumular 10 puntos de rendimiento general", 10,
               "rendimiento_general"),
    LOGRO_SERIE_NIVELES_5("Rendimiento", "Acumular", "puntos de rendimiento general",
                          "rendimiento_general", 50, 100, 250, 500, 1000),
    // Mood
    LOGRO_ITEM("Animo Inicial", "Acumular 10 puntos de estado de Animo", 10, "estado_animo"),
    LOGRO_SERIE_NIVELES_5("Animo", "Acumular", "puntos de estado de Animo", "estado_animo", 50, 100,
                          250, 500, 1000),
    // Distinct Pitches
    LOGRO_ITEM("Explorador de Canchas", "Jugar en 1 cancha distinta", 1, "canchas_distintas"),
    LOGRO_SERIE_NIVELES_4("Viajero", "Jugar en", "canchas distintas", "canchas_distintas", 5, 10,
                          25, 50),
    // Hat-Tricks
    LOGRO_ITEM("Primer Hat-Trick", "Anotar 3 o mas goles en un partido", 1, "hat_tricks"),
    LOGRO_SERIE_NIVELES_3("Hat-Tricker", "Anotar 3 o mas goles en", "partidos", "hat_tricks", 5, 10,
                          25),
    // Poker Assists
    LOGRO_ITEM("Primer Poker de Asistencias", "Dar 4 o mas asistencias en un partido", 1,
               "poker_asistencias"),
    LOGRO_SERIE_NIVELES_2("Poker Asistente", "Dar 4 o mas asistencias en", "partidos",
                          "poker_asistencias", 5, 10),
    // Perfect Performance
    LOGRO_ITEM("Primer Rendimiento Perfecto", "Obtener rendimiento perfecto (10) en un partido", 1,
               "rendimiento_perfecto"),
    LOGRO_SERIE_NIVELES_3("Rendimiento Perfecto", "Obtener rendimiento perfecto en", "partidos",
                          "rendimiento_perfecto", 5, 10, 25),
    // Perfect Mood
    LOGRO_ITEM("Primer Animo Perfecto", "Obtener animo perfecto (10) en un partido", 1,
               "animo_perfecto"),
    LOGRO_SERIE_NIVELES_3("Animo Perfecto", "Obtener animo perfecto en", "partidos",
                          "animo_perfecto", 5, 10, 25),
    // Victory Achievements
    LOGRO_ITEM("Goleador Victorioso", "Anotar 10 goles en partidos ganados", 10, "goles_victorias"),
    LOGRO_ITEM("Asistente Victorioso", "Dar 10 asistencias en partidos ganados", 10,
               "asistencias_victorias"),
    LOGRO_ITEM("Rendimiento Victorioso", "Acumular 50 puntos de rendimiento en victorias", 50,
               "rendimiento_victorias"),
    LOGRO_ITEM("Animo Victorioso", "Acumular 50 puntos de animo en victorias", 50,
               "animo_victorias"),
    // Loss Achievements
    LOGRO_ITEM("Goleador en Derrotas", "Anotar 5 goles en partidos perdidos", 5, "goles_derrotas"),
    LOGRO_ITEM("Asistente en Derrotas", "Dar 5 asistencias en partidos perdidos", 5,
               "asistencias_derrotas"),
    // Draw Achievements
    LOGRO_ITEM("Rendimiento en Empates", "Acumular 25 puntos de rendimiento en empates", 25,
               "rendimiento_empates"),
    ...
    LOGRO_ITEM("Asistencia en Derrota", "Asistir en 5 partidos perdidos", 5,
               "asistencias_derrotas"),
    ...
    LOGRO_ITEM("Rendimiento en Derrotas", "Acumular 50 puntos de rendimiento en derrotas", 50,
               "rendimiento_derrotas"),
    LOGRO_ITEM("Animo en Derrotas", "Acumular 50 puntos de animo en derrotas", 50,
               "animo_derrotas"),
    // No Contribution Achievements
    LOGRO_PAR("Primer Partido Sin Goles", "Jugar un partido sin anotar", 1, "5 Partidos Sin Goles",
              "Jugar 5 partidos sin anotar", 5, "partidos_sin_goles"),
    ...
    LOGRO_PAR("Primer Gol Anotado", "Anotar en un partido", 1, "5 Partidos con Goles",
              "Anotar en 5 partidos", 5, "partidos_con_goles"),
    LOGRO_PAR("Primer Asistencia Dada", "Asistir en un partido", 1, "5 Partidos con Asistencias",
              "Asistir en 5 partidos", 5, "partidos_con_asistencias"),
    LOGRO_PAR("Contribuidor Inicial", "Contribuir en un partido", 1, "Contribuidor Regular",
              "Contribuir en 10 partidos", 10, "partidos_con_contribucion"),
    // Advanced Scoring
    ...
    LOGRO_PAR("Rendimiento Alto Inicial", "Obtener rendimiento >=8 en un partido", 1,
              ...
              "rendimiento_alto"),
    LOGRO_PAR("Animo Alto Inicial", "Obtener animo >=8 en un partido", 1, "Animo Alto Regular",
              "Obtener animo >=8 en 10 partidos", 10, "animo_alto"),
    // Low Performance
    ...
    LOGRO_ITEM("Promedio Goleador", "Mantener promedio de 0.5 goles por partido", 5,
               "goles_por_partido_promedio"),
    LOGRO_ITEM("Promedio Asistente", "Mantener promedio de 0.5 asistencias por partido", 5,
               "asistencias_por_partido_promedio"),
    LOGRO_ITEM("Promedio Rendimiento Alto", "Mantener promedio de rendimiento >=7", 70,
               "rendimiento_promedio"),
    LOGRO_ITEM("Promedio Animo Alto", "Mantener promedio de animo >=7", 70, "animo_promedio"),
    // Near Perfect
    LOGRO_ITEM("Rendimiento Cercano a Perfecto", "Obtener rendimiento >=9 en un partido", 1,
               "partidos_con_rendimiento_alto"),
    LOGRO_ITEM("Animo Cercano a Perfecto", "Obtener animo >=9 en un partido", 1,
               "partidos_con_animo_alto"),
    LOGRO_ITEM("Dia Perfecto", "Obtener rendimiento y animo perfectos en un partido", 1,
               "partidos_con_rendimiento_perfecto_y_animo"),
    // Placeholder for time-based (since no time columns)
    LOGRO_ITEM("Goleador en Primer Tiempo", "Anotar 10 goles (simulado)", 10,
               "goles_en_primer_tiempo"),
    LOGRO_ITEM("Asistente en Segundo Tiempo", "Dar 10 asistencias (simulado)", 10,
               "asistencias_en_segundo_tiempo"),
    ...
    LOGRO_ITEM("ultima Asistencia", "Asistir en el ultimo partido", 1,
               "asistencias_en_ultimo_partido"),
    LOGRO_ITEM("Ultimo Rendimiento Perfecto", "Rendimiento perfecto en el ultimo partido", 10,
               "rendimiento_en_ultimo_partido"),
    LOGRO_ITEM("Ultimo Animo Perfecto", "Animo perfecto en el ultimo partido", 10,
               "animo_en_ultimo_partido"),
    // More Tiered Achievements
    LOGRO_EXPERTO("Goleador en Empates", "Anotar 10 goles en empates", 10, "goles_empates"),
    LOGRO_EXPERTO("Asistente en Empates", "Dar 10 asistencias en empates", 10,
                  "asistencias_empates"),
    LOGRO_EXPERTO("Rendimiento en Derrotas", "Acumular 100 puntos de rendimiento en derrotas", 100,
                  "rendimiento_derrotas"),
    LOGRO_EXPERTO("Animo en Derrotas", "Acumular 100 puntos de animo en derrotas", 100,
                  "animo_derrotas"),
    LOGRO_ITEM("10 Partidos Sin Goles", "Jugar 10 partidos sin anotar", 10, "partidos_sin_goles"),
    LOGRO_ITEM("10 Partidos Sin Asistencias", "Jugar 10 partidos sin asistir", 10,
               "partidos_sin_asistencias"),
    LOGRO_ITEM("10 Partidos con Goles", "Anotar en 10 partidos", 10, "partidos_con_goles"),
    LOGRO_ITEM("10 Partidos con Asistencias", "Asistir en 10 partidos", 10,
               "partidos_con_asistencias"),
    LOGRO_ITEM("Contribuidor Avanzado", "Contribuir en 25 partidos", 25,
               "partidos_con_contribucion"),
    LOGRO_EXPERTO("Hat-Tricker Doble", "Anotar 4 o mas goles en 10 partidos", 10,
                  "hat_tricks_dobles"),
    LOGRO_EXPERTO("Poker Asistente Doble", "Dar 5 o mas asistencias en 10 partidos", 10,
                  "asistencias_dobles"),
    LOGRO_EXPERTO("Rendimiento Alto", "Obtener rendimiento >=8 en 25 partidos", 25,
                  "rendimiento_alto"),
    LOGRO_EXPERTO("Animo Alto", "Obtener animo >=8 en 25 partidos", 25, "animo_alto"),
    LOGRO_EXPERTO("Rendimiento Bajo", "Obtener rendimiento <=3 en 5 partidos", 5,
                  "rendimiento_bajo"),
    LOGRO_EXPERTO("Animo Bajo", "Obtener animo <=3 en 5 partidos", 5, "animo_bajo"),
    LOGRO_EXPERTO("Promedio Goleador", "Mantener promedio de 1 gol por partido", 10,
                  "goles_por_partido_promedio"),
    LOGRO_EXPERTO("Promedio Asistente", "Mantener promedio de 1 asistencia por partido", 10,
                  "asistencias_por_partido_promedio"),
    LOGRO_EXPERTO("Rendimiento Cercano a Perfecto", "Obtener rendimiento >=9 en 10 partidos", 10,
                  "partidos_con_rendimiento_alto"),
    LOGRO_EXPERTO("Animo Cercano a Perfecto", "Obtener animo >=9 en 10 partidos", 10,
                  "partidos_con_animo_alto"),
    LOGRO_EXPERTO("Dia Perfecto", "Obtener rendimiento y animo perfectos en 5 partidos", 5,
                  "partidos_con_rendimiento_perfecto_y_animo"),
...
static void listar_camisetas_con_partidos(void)
{
    ...
    if (!db_prepare_stmt_with_error(&stmt,
                                    "SELECT DISTINCT c.id, c.nombre FROM camiseta c INNER JOIN "
                                    "partido p ON c.id = p.camiseta_id ORDER BY c.id",
...
#define LOGROS_FILTROS(X)                                                                          \
    ...
    X(2, mostrar_logros_completados, "LOGROS COMPLETADOS", LOGRO_FILTRO_COMPLETADOS,               \
      "Logros Completados")                                                                        \
    X(3, mostrar_logros_en_progreso, "LOGROS EN PROGRESO", LOGRO_FILTRO_EN_PROGRESO,               \
      "Logros en Progreso")                                                                        \
    X(4, mostrar_logros_no_completados, "LOGROS NO COMPLETADOS", LOGRO_FILTRO_NO_COMPLETADOS,      \
      "Logros No Completados")
