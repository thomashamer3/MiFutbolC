...
static int export_pdf_strncpy_s(char *dest, size_t destsz, const char *src)
{
    ...
    return strncpy_s(dest, destsz, src, _TRUNCATE);
#else
    /* Use strlen_s (Annex K compatible) to avoid reading beyond `destsz` bytes if `src` is not
     * NUL-terminated. */
...
static int pdf_add_text_safe(PdfCtx *ctx, struct pdf_object *page, const char *text, float size,
                             float x, float y, uint32_t colour)
{
...
static int pdf_get_text_width_safe(PdfCtx *ctx, const char *font, const char *text, float size,
                                   float *width)
{
...
static void new_page(PdfCtx *ctx)
{
    ...
    if (ctx->page_cap <= ctx->page_count)
    {
        ...
        struct pdf_object **new_pages = (struct pdf_object **)realloc(
                                            (void *)ctx->pages, sizeof(struct pdf_object *) * new_cap);
        if (new_pages)
        {
            ctx->pages = new_pages;
...
static void write_text_line(PdfCtx *ctx, const char *text, const char *font, float size,
                            float leading)
{
    if (!text)
    {
...
static void ejecutar_consulta_una_fila(PdfCtx *ctx, const char *sql, const char *mes_yyyy_mm,
                                       int bind_count, PdfRowHandler row_handler)
{
...
static void manejar_fila_resumen_partidos(PdfCtx *ctx, sqlite3_stmt *stmt)
{
    ...
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
    snprintf(line, sizeof(line), "Rendimiento: %.2f | cansancio: %.2f | animo: %.2f", avg_rend,
             avg_cans, avg_animo);
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
}
...
{
    write_text_line(ctx, "Resumen de Partidos", ctx->font_bold, PDF_SECTION_SIZE,
                    PDF_SECTION_SIZE + 4.0F);

    const char *sql_partidos = "SELECT COUNT(*), AVG(goles), AVG(asistencias), "
...
static void manejar_fila_resumen_entrenamiento(PdfCtx *ctx, sqlite3_stmt *stmt)
{
    ...
    char line[256];
    snprintf(line, sizeof(line), "Sesiones: %d | minutos: %d | intensidad promedio: %.2f", total,
             total_min, avg_int);
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
...
static void escribir_resumen_entrenamiento(PdfCtx *ctx, const char *mes_yyyy_mm)
{
    ...
    write_text_line(ctx, "Resumen de Entrenamiento", ctx->font_bold, PDF_SECTION_SIZE,
                    PDF_SECTION_SIZE + 4.0F);
    ...
    ejecutar_consulta_una_fila(ctx, sql_entrenamiento, mes_yyyy_mm, 1,
                               manejar_fila_resumen_entrenamiento);
}
...
static void manejar_fila_resumen_alimentacion(PdfCtx *ctx, sqlite3_stmt *stmt)
{
    ...
    char line[256];
    snprintf(line, sizeof(line), "Comidas: %d | Buenas: %d | Regulares: %d | Malas: %d", total,
...
{
    write_blank_line(ctx, PDF_BODY_SIZE + 4.0F);
    write_text_line(ctx, "Resumen de Alimentacion", ctx->font_bold, PDF_SECTION_SIZE,
                    PDF_SECTION_SIZE + 4.0F);

    const char *sql_alimentacion = "SELECT COUNT(*), "
    ...
    ejecutar_consulta_una_fila(ctx, sql_alimentacion, mes_yyyy_mm, 1,
                               manejar_fila_resumen_alimentacion);

    const char *sql_hidratacion = "SELECT "
    ...
    ejecutar_consulta_una_fila(ctx, sql_hidratacion, mes_yyyy_mm, 1,
                               manejar_fila_resumen_hidratacion);
}
...
static void manejar_fila_resumen_mental(PdfCtx *ctx, sqlite3_stmt *stmt)
{
    ...
    char line[256];
    snprintf(line, sizeof(line), "Sesiones: %d | confianza %.2f | estres %.2f", total, conf,
             estres);
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
    snprintf(line, sizeof(line), "Motivacion %.2f | presion %.2f | concentracion %.2f", mot, pres,
             conc);
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
}
...
static void manejar_fila_resumen_tendencia(PdfCtx *ctx, sqlite3_stmt *stmt)
{
    ...
    char line[256];
    snprintf(line, sizeof(line), "Rendimiento con confianza alta: %.2f | baja: %.2f", avg_alta,
             avg_baja);
    write_text_line(ctx, line, ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);

    if (avg_alta > avg_baja)
    {
        write_text_line(ctx, "Tendencia: Jugas mejor cuando la confianza esta alta.",
                        ctx->font_regular, PDF_BODY_SIZE, PDF_BODY_SIZE + 2.0F);
    }
}
...
{
    write_blank_line(ctx, PDF_BODY_SIZE + 4.0F);
    write_text_line(ctx, "Resumen de Mentalidad", ctx->font_bold, PDF_SECTION_SIZE,
                    PDF_SECTION_SIZE + 4.0F);

    const char *sql_mental = "SELECT COUNT(*), AVG(confianza), AVG(estres), AVG(motivacion), "
                             "AVG(presion), AVG(concentracion) "
    ...
    const char *sql_tendencia = "SELECT "
                                "(SELECT AVG(p.rendimiento_general) FROM partido p "
                                " WHERE substr(p.fecha, 1, 7) = ? AND p.fecha IN "
                                " (SELECT DISTINCT fecha FROM bienestar_sesion_mental WHERE "
                                "confianza >= 8 AND substr(fecha, 1, 7) = ?)) AS avg_alta, "
                                "(SELECT AVG(p.rendimiento_general) FROM partido p "
                                " WHERE substr(p.fecha, 1, 7) = ? AND p.fecha IN "
                                " (SELECT DISTINCT fecha FROM bienestar_sesion_mental WHERE "
                                "confianza <= 4 AND substr(fecha, 1, 7) = ?)) AS avg_baja";
...
static void write_long_word(PdfCtx *ctx, const char *word, const char *font, float size,
                            float leading, float max_width)
{
    char chunk[256];
    size_t clen = 0;
    ...
    for (size_t i = 0; word && word[i] != '\0'; i++)
    {
        ...
        if (pdf_get_text_width_safe(ctx, font, chunk, size, &chunk_width) == 0 &&
                chunk_width > max_width && clen > 1)
        {
            chunk[clen - 1] = '\0';
            write_text_line(ctx, chunk, font, size, leading);
            clen = 0;
...
static int try_append_word(PdfCtx *ctx, char *line, size_t line_size, const char *word,
                           const char *font, float size, float max_width)
{
...
static void write_wrapped_text(PdfCtx *ctx, const char *text, const char *font, float size,
                               float leading)
{
    if (!text || text[0] == '\0')
    {
...
static void write_section_header(PdfCtx *ctx, const char *title)
{
    ...
    pdf_add_filled_rectangle(ctx->pdf, ctx->page, ctx->margin, band_y,
                             page_width - (2.0F * ctx->margin), band_height, 0.0F,
                             PDF_RGB(235, 240, 250), PDF_RGB(235, 240, 250));

    pdf_set_font(ctx->pdf, ctx->font_bold);
    pdf_add_text_safe(ctx, ctx->page, title, PDF_SECTION_SIZE, ctx->margin + 6.0F, ctx->y,
                      PDF_BLACK);
...
static void add_page_footers(PdfCtx *ctx)
{
    ...
    for (int i = 0; i < pages; i++)
    {
        ...
        pdf_add_line(ctx->pdf, page, ctx->margin, y + 6.0F, page_width - ctx->margin, y + 6.0F,
                     0.3F, PDF_RGB(204, 204, 204));

        pdf_set_font(ctx->pdf, ctx->font_regular);
        float text_width = 0.0F;
        pdf_get_text_width_safe(ctx, ctx->font_regular, footer, PDF_FOOTER_SIZE, &text_width);
        pdf_add_text_safe(ctx, page, footer, PDF_FOOTER_SIZE, (page_width - text_width) / 2.0F, y,
                          PDF_BLACK);
    }
}
                       ...
                       const char *datetime, const char *export_dir, int total_sections)
{
    ...
    pdf_add_filled_rectangle(ctx->pdf, ctx->page, 0, page_height - 140.0F, page_width, 140.0F, 0.0F,
                             PDF_RGB(26, 89, 166), PDF_RGB(26, 89, 166));

    pdf_set_font(ctx->pdf, ctx->font_bold);
    float title_width = 0.0F;
    pdf_get_text_width_safe(ctx, ctx->font_bold, title, 24.0F, &title_width);
    pdf_add_text_safe(ctx, ctx->page, title, 24.0F, (page_width - title_width) / 2.0F,
                      page_height - 85.0F, PDF_WHITE);

    pdf_set_font(ctx->pdf, ctx->font_regular);
    float sub_width = 0.0F;
    pdf_get_text_width_safe(ctx, ctx->font_regular, subtitle, PDF_SUBTITLE_SIZE, &sub_width);
    pdf_add_text_safe(ctx, ctx->page, subtitle, PDF_SUBTITLE_SIZE, (page_width - sub_width) / 2.0F,
                      page_height - 110.0F, PDF_WHITE);

    ctx->y = page_height - 190.0F;

    write_text_line(ctx, "Resumen del informe", ctx->font_bold, PDF_SECTION_SIZE,
                    PDF_SECTION_SIZE + 6.0F);
    write_blank_line(ctx, PDF_BODY_SIZE + 2.0F);
...
static void procesar_archivo_txt(const char *path, const char *titulo, PdfCtx *ctx)
{
    ...
    if (err != 0 || !file)
    {
        write_wrapped_text(ctx, "[Archivo no disponible]", ctx->font_italic, PDF_BODY_SIZE,
                           PDF_BODY_SIZE + 3.0F);
...
int generar_informe_total_pdf(void)
{
    ...
    const char *files[] = {"camisetas_mejorado.txt",
                           "partidos.txt",
    ...
    const char *titles[] = {"Camisetas (analisis avanzado)",
                            "Partidos",
    ...
    int flag = escribir_pdf(pdf_path, &ctx);
    cleanup_pdf_ctx(&ctx);
...
int generar_informe_personal_mensual_pdf(const char *mes_yyyy_mm)
{
    ...
    write_text_line(&ctx, "Este informe es 100% local.", ctx.font_italic, PDF_BODY_SIZE,
                    PDF_BODY_SIZE + 2.0F);
