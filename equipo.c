...
static int cargar_imagen_para_equipo_id(int id)
{
    ...
    if (!app_select_image_from_user(ruta_origen, sizeof(ruta_origen), "mifutbol_imagen_sel_equipo.txt"))
    {
    ...
    snprintf(base_destino, sizeof(base_destino), "equipo_%d_%s", id, ts);
    ...
    sqlite3_stmt *stmt;
    if (!preparar_stmt(&stmt, "UPDATE equipo SET imagen_ruta=? WHERE id=?"))
    {
