...
static const char *SQL_STATS_BY_ANIO =
    "SELECT substr(fecha_hora, 7, 4) AS anio, c.nombre, COUNT(*) AS partidos, SUM(goles) AS "
    "total_goles, SUM(asistencias) AS total_asistencias, ROUND(AVG(goles), 2) AS avg_goles, "
...
static void write_stats_anio_common(FILE *file, anio_header_writer_t write_header,
                                    anio_row_writer_t write_row, anio_footer_writer_t write_footer)
{
    sqlite3_stmt *stmt;
...
{
    write_stats_anio_common(file, write_stats_anio_txt_header, write_stats_anio_txt_row, NULL);
}

static void write_stats_anio_html(FILE *file)
{
    write_stats_anio_common(file, write_stats_anio_html_header, write_stats_anio_html_row,
...
void exportar_estadisticas_csv(void)
{
    ...
    FILE *file = abrir_archivo_exportacion("estadisticas.csv", "Error al crear el archivo CSV");
    if (!file)
    {
        return;
    }

    fprintf(file, "Camiseta,Goles,Asistencias,Partidos,Victorias,Empates,Derrotas\n");
    write_stats_csv(file);
...
void exportar_estadisticas_txt(void)
{
    ...
    FILE *file = abrir_archivo_exportacion("estadisticas.txt", "Error al crear el archivo TXT");
    if (!file)
    {
        return;
    }

    write_stats_txt(file);
...
void exportar_estadisticas_json(void)
{
    ...
    FILE *file = abrir_archivo_exportacion("estadisticas.json", "Error al crear el archivo JSON");
    if (!file)
    {
        return;
    }

    write_stats_json(file);
...
void exportar_estadisticas_html(void)
{
    ...
    FILE *file = abrir_archivo_exportacion("estadisticas.html", "Error al crear el archivo HTML");
    if (!file)
    {
        return;
    }
...
void exportar_estadisticas_md(void)
{
    ...
    FILE *file = abrir_archivo_exportacion("estadisticas.md", "Error al crear el archivo Markdown");
    if (!file)
    {
        return;
    }

    fprintf(file, "# Estadisticas por Camiseta\n\n");
...
void exportar_estadisticas_por_anio_csv(void)
{
    ...
    FILE *file =
        abrir_archivo_exportacion("estadisticas_por_anio.csv", "Error al crear el archivo CSV");
    if (!file)
    {
        return;
    }

    fprintf(file, "Anio,Camiseta,Partidos,Goles,Asistencias,Promedio Goles,Promedio Asistencias\n");
    write_stats_anio_csv(file);
...
void exportar_estadisticas_por_anio_txt(void)
{
    ...
    FILE *file =
        abrir_archivo_exportacion("estadisticas_por_anio.txt", "Error al crear el archivo TXT");
    if (!file)
    {
        return;
    }

    write_stats_anio_txt(file);
...
void exportar_estadisticas_por_anio_json(void)
{
    ...
    FILE *file =
        abrir_archivo_exportacion("estadisticas_por_anio.json", "Error al crear el archivo JSON");
    if (!file)
    {
        return;
    }

    write_stats_anio_json(file);
...
void exportar_estadisticas_por_anio_html(void)
{
    ...
    FILE *file =
        abrir_archivo_exportacion("estadisticas_por_anio.html", "Error al crear el archivo HTML");
    if (!file)
    {
        return;
    }
...
static void stats_camiseta_csv_rows(FILE *file, sqlite3_stmt *stmt)
{
    fprintf(file, "Camiseta,Goles,Asistencias,Partidos,Victorias,Empates,Derrotas\n");
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
static void stats_camiseta_txt_rows(FILE *file, sqlite3_stmt *stmt)
{
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        fprintf(file, "%s | G:%d A:%d P:%d V:%d E:%d D:%d\n", sqlite3_column_text(stmt, 0),
...
static void stats_camiseta_html_rows(FILE *file, sqlite3_stmt *stmt)
{
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        fprintf(file,
...
static void stats_camiseta_json_rows(FILE *file, sqlite3_stmt *stmt)
{
    cJSON *root = cJSON_CreateArray();
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
static void stats_camiseta_md_rows(FILE *file, sqlite3_stmt *stmt)
{
    fprintf(file,
...
static void stats_anio_csv_rows(FILE *file, sqlite3_stmt *stmt)
{
    fprintf(file, "Anio,Camiseta,Partidos,Goles,Asistencias,Promedio Goles,Promedio Asistencias\n");
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
static void stats_anio_txt_rows(FILE *file, sqlite3_stmt *stmt)
{
    stats_anio_common_from_stmt(file, stmt, write_stats_anio_txt_header, write_stats_anio_txt_row,
                                NULL);
}

static void stats_anio_html_rows(FILE *file, sqlite3_stmt *stmt)
{
    stats_anio_common_from_stmt(file, stmt, write_stats_anio_html_header, write_stats_anio_html_row,
                                write_stats_anio_html_footer);
}

static void stats_anio_json_rows(FILE *file, sqlite3_stmt *stmt)
{
    cJSON *root = cJSON_CreateObject();
    char current_anio[5] = "";
    cJSON *anio_arr = NULL;
...
void exportar_estadisticas_all(void)
{
    ...
    file = abrir_archivo_exportacion("estadisticas.csv", "Error al crear CSV");
    if (file)
    {
        stats_camiseta_csv_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas.csv"));
        fclose(file);
    }

    sqlite3_reset(stmt);
    file = abrir_archivo_exportacion("estadisticas.txt", "Error al crear TXT");
    if (file)
    {
        stats_camiseta_txt_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas.txt"));
        fclose(file);
    }

    sqlite3_reset(stmt);
    file = abrir_archivo_exportacion("estadisticas.json", "Error al crear JSON");
    if (file)
    {
        stats_camiseta_json_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas.json"));
        fclose(file);
    }
    ...
    sqlite3_reset(stmt);
    file = abrir_archivo_exportacion("estadisticas.md", "Error al crear MD");
    if (file)
    {
        fprintf(file, "# Estadisticas por Camiseta\n\n*Generado por MiFutbolC*\n\n");
        stats_camiseta_md_rows(file, stmt);
        fprintf(file, "\n");
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas.md"));
        fclose(file);
    }
...
void exportar_estadisticas_por_anio_all(void)
{
    ...
    file = abrir_archivo_exportacion("estadisticas_por_anio.csv", "Error al crear CSV");
    if (file)
    {
        stats_anio_csv_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas_por_anio.csv"));
        fclose(file);
    }

    sqlite3_reset(stmt);
    file = abrir_archivo_exportacion("estadisticas_por_anio.txt", "Error al crear TXT");
    if (file)
    {
        stats_anio_txt_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas_por_anio.txt"));
        fclose(file);
    }

    sqlite3_reset(stmt);
    file = abrir_archivo_exportacion("estadisticas_por_anio.json", "Error al crear JSON");
    if (file)
    {
        stats_anio_json_rows(file, stmt);
        printf("Archivo exportado a: %s\n", get_export_path("estadisticas_por_anio.json"));
        fclose(file);
    }
