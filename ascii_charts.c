...
static void print_line_char(char character, int length)
{
    for (int i = 0; i < length; i++)
// NOLINTEND(bugprone-easily-swappable-parameters)
    {
        ui_putchar(character);
    }
}
...
static char elegir_caracter_linea(const int *rows, int col, int width)
{
    if (width <= 1)
    {
        return '-';
    ...
    {
        return caracter_conexion_medio(rows[col], rows[col - 1], rows[col + 1]);
    }
        ...
        double max_val, double range)
{
    ...
    {
        double interpol_pos = (count > 1) ? (double)col / (width - 1) * (count - 1) : 0.0;
        int idx = (int)interpol_pos;
        double frac = interpol_pos - idx;
        ...
        int row = round_to_int((max_val - val) / range * (height - 1));
        if (row < 0)
        {
            row = 0;
        }
 ...
 */
static void imprimir_fila_grafico_lineas(int row, const LineChartRowConfig *cfg)
{
    ...
    for (int col = 0; col < cfg->width; col++)
    {
        if (cfg->rows[col] == row)
        {
            line[col] = elegir_caracter_linea(cfg->rows, col, cfg->width);
        }
    }
    ...
    for (int i = 0; i < cfg->count; i++)
    {
        ...
        if (cfg->count > 1)
        {
            col = round_to_int((double)i / (cfg->count - 1) * (cfg->width - 1));
        }
        else
        {
            col = 0;
        ...
        if (col >= 0 && col < cfg->width && cfg->rows[col] == row)
        {
            line[col] = '*';
        }
    }
    ...
    if (row == 0)
    {
        ylabel = cfg->label_max;
    }
    else if (row == cfg->height - 1)
    {
        ylabel = cfg->label_min;
    }

    ui_printf(" %*s | ", cfg->label_w, ylabel);
    for (int col = 0; col < cfg->width; col++)
    {
        ui_putchar(line[col]);
    }
                            ...
                            int width, int height)
{
    ...
    {
        imprimir_fila_grafico_lineas(row, &lcfg);
    }
 ...
 */
static void imprimir_fila_histograma(int index, const HistogramRowConfig *cfg)
{
    double bin_lo = cfg->min_val + (index * cfg->bin_width);
    double bin_hi = bin_lo + cfg->bin_width;

    char range_label[32];
    if (index == cfg->bins - 1)
    {
        snprintf(range_label, sizeof(range_label), "[%.2f, %.2f]", bin_lo, bin_hi);
    }
    else
    {
        snprintf(range_label, sizeof(range_label), "[%.2f, %.2f)", bin_lo, bin_hi);
    ...
    int bar_len = round_to_int((double)cfg->freq[index] / cfg->max_freq * cfg->bar_max);
    if (bar_len < 0)
    {
        bar_len = 0;
    }

    ui_printf(" %-26s | %4d ", range_label, cfg->freq[index]);
    for (int j = 0; j < bar_len; j++)
    {
