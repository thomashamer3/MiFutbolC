...
{
    (void)context;
    fprintf(file, "id,nombre,tiene_equipo_fijo,cantidad_equipos,tipo,formato\n");
}
...
{
    (void)context;
    fprintf(file, "LISTADO DE TORNEOS\n\n");
}
...
static void write_json_row(FILE *file, sqlite3_stmt *stmt, void *context) /* NOSONAR */
{
    ...
    cJSON_AddStringToObject(item, "formato_torneo",
                            get_nombre_formato_torneo((FormatoTorneos)formato));
...
EXPORT_FORMAT_ROWS(exportar_torneos_csv, obtener_datos_torneos, "torneos.csv", NULL,
                   write_csv_header, write_csv_row, NULL)
EXPORT_FORMAT_ROWS(exportar_torneos_txt, obtener_datos_torneos, "torneos.txt", NULL,
