...
static void zip_write_local_header(FILE *file, const char *name, uint32_t crc32val, uint32_t size,
                                   uint16_t dos_time, uint16_t dos_date, uint32_t *offset_out)
{
    ...
    fwrite(hdr, 1, 30, file);
    fwrite(name, 1, name_len, file);
}

static void zip_write_central_dir(FILE *file, const char *name, uint32_t crc32val, uint32_t size,
                                  uint16_t dos_time, uint16_t dos_date, uint32_t local_offset)
{
    uint16_t name_len = (uint16_t)strnlen_s(name, (size_t)-1);
    uint8_t cd[46] = {0};
    cd[0] = 'P';
...
static void zip_write_end(FILE *file, uint32_t cd_offset, uint32_t cd_size, uint16_t entries)
{
    uint8_t eocd[22] = {0};
    eocd[0] = 'P';
    eocd[1] = 'K';
...
static uint16_t dos_date_now(void)
{
    ...
    return (uint16_t)(local_tm.tm_mday | ((local_tm.tm_mon + 1) << 5) |
                      ((local_tm.tm_year - 80) << 9));
}
...
static void ods_escape_xml(FILE *file, const char *texto)
{
    if (!texto)
    {
        return;
...
static void ods_escribir_fila_texto(FILE *file, const char *celdas[], int num_celdas)
{
    fprintf(file, "  <table:table-row>\n");
    for (int i = 0; i < num_celdas; i++)
    {
        fprintf(file, "   <table:table-cell office:value-type=\"string\"><text:p>");
        ods_escape_xml(file, celdas[i] ? celdas[i] : "");
        fprintf(file, "</text:p></table:table-cell>\n");
    }
    fprintf(file, "  </table:table-row>\n");
}

static int ods_exportar_tabla(const char *nombre_hoja, const char *sql, const char *encabezados[],
                              int num_cols)
{
    char filename[256];
    ...
    fprintf(file, "<office:document-content "
            "xmlns:office=\"urn:oasis:names:tc:opendocument:xmlns:office:1.0\" "
            "xmlns:table=\"urn:oasis:names:tc:opendocument:xmlns:table:1.0\" "
            "xmlns:text=\"urn:oasis:names:tc:opendocument:xmlns:text:1.0\">\n");
    fprintf(file, "<office:body><office:spreadsheet>\n");
    fprintf(file, " <table:table table:name=\"%s\">\n", nombre_hoja);

    ods_escribir_fila_texto(file, encabezados, num_cols);
    ...
    fprintf(file, " </table:table>\n");
    fprintf(file, "</office:spreadsheet></office:body>\n");
    fprintf(file, "</office:document-content>\n");
    fclose(file);
...
{
    const char *headers[] = {"Fecha", "Goles", "Asistencias", "Resultado",
                             "Clima", "Dia",   "Cancha"
                            };
    return ods_exportar_tabla(
               "Partidos",
               "SELECT p.fecha_hora, p.goles, p.asistencias, p.resultado, p.clima, p.dia, can.nombre "
               "FROM partido p LEFT JOIN cancha can ON p.cancha_id = can.id WHERE p.resultado > 0 ORDER "
               "BY p.fecha_hora DESC",
               headers, 7);
}
static int exportar_equipos_ods_local(void)
{
    ...
    return ods_exportar_tabla(
               "Equipos", "SELECT nombre, pais, ciudad, estadio FROM equipo ORDER BY nombre", headers, 4);
}
static int exportar_canchas_ods_local(void)
{
    const char *headers[] = {"Nombre", "Ciudad", "Capacidad"};
    return ods_exportar_tabla(
               "Canchas", "SELECT nombre, ciudad, capacidad FROM cancha ORDER BY nombre", headers, 3);
}
static int exportar_camisetas_ods_local(void)
{
    const char *headers[] = {"Nombre", "Club", "Temporada", "Talle"};
    return ods_exportar_tabla(
               "Camisetas", "SELECT nombre, club, temporada, talle FROM camisetas ORDER BY nombre",
               headers, 4);
}
...
static int exportar_torneo_ods_local(void)
{
    ...
    return ods_exportar_tabla(
               "Torneos",
               "SELECT nombre, tipo, fecha_inicio, fecha_fin, estado FROM torneo ORDER BY nombre", headers,
               5);
}
...
{
    MenuItem items[] = {{1, "Exportar Partidos a ODS", &menu_exportar_partidos_ods_fn},
...
static void dynbuf_init(DynBuf *buffer)
{
    buffer->data = NULL;
    buffer->len = 0;
    buffer->cap = 0;
...
static int dynbuf_write(DynBuf *buffer, const void *src, size_t n)
{
    if (n == 0)
    {
        return 1;
    }
    if (n > SIZE_MAX - buffer->len)
    {
        return 0;
    }
...
static int dynbuf_printf(DynBuf *buffer, const char *fmt, ...) // NOSONAR
{
    char tmp[4096];
    va_list ap;
    va_start(ap, fmt);
    ...
    return dynbuf_write(buffer, tmp, (size_t)n);
}

static void dynbuf_free(DynBuf *buffer)
{
    free(buffer->data);
    buffer->data = NULL;
    buffer->len = 0;
...
static void xlsx_escape_xml_buf(DynBuf *buffer, const char *texto)
{
    if (!texto)
    {
        return;
    ...
    for (const char *p = texto; *p; p++)
    {
        ...
        {
        case '&':
            dynbuf_write(buffer, "&amp;", 5);
            break;
        case '<':
            dynbuf_write(buffer, "&lt;", 4);
            break;
        case '>':
            dynbuf_write(buffer, "&gt;", 4);
            break;
        case '"':
            dynbuf_write(buffer, "&quot;", 6);
            break;
        case '\'':
            dynbuf_write(buffer, "&apos;", 6);
            break;
        default:
            dynbuf_write(buffer, p, 1);
            break;
        }
    }
}
...
static int xlsx_generar_xml(const char *nombre_hoja, const char *sql, const char *encabezados[],
                            int num_cols, DynBuf *buf)
{
...
static int xlsx_exportar_tabla(const char *nombre_hoja, const char *sql, const char *encabezados[],
                               int num_cols)
{
    char filename[256];
    ...
    zip_write_local_header(file, entry_name, crc, (uint32_t)buf.len, dtime, ddate, &local_offset);
    ...
    snprintf(types_xml, sizeof(types_xml),
             ...
             "<Override PartName=\"/xl/content.xml\" "
             "ContentType=\"application/vnd.ms-excel.sheet.macroEnabled.main+xml\"/>"
             "</Types>");
    uint32_t types_crc = crc32_buf((const uint8_t *)types_xml, strnlen_s(types_xml, (size_t)-1));
    uint32_t types_offset;
    zip_write_local_header(file, types_name, types_crc, (uint32_t)strnlen_s(types_xml, (size_t)-1),
                           dtime, ddate, &types_offset);
    fwrite(types_xml, 1, strnlen_s(types_xml, (size_t)-1), file);

    // Central directory
    uint32_t cd_offset = (uint32_t)ftell(file);
    uint32_t cd_size = 0;

    zip_write_central_dir(file, entry_name, crc, (uint32_t)buf.len, dtime, ddate, local_offset);
    cd_size += 46 + (uint16_t)strnlen_s(entry_name, (size_t)-1);
    zip_write_central_dir(file, types_name, types_crc, (uint32_t)strnlen_s(types_xml, (size_t)-1),
                          dtime, ddate, types_offset);
    cd_size += 46 + (uint16_t)strnlen_s(types_name, (size_t)-1);

    zip_write_end(file, cd_offset, cd_size, 2);
...
static int exportar_partidos_xlsx_local(void)
{
    ...
    return xlsx_exportar_tabla(
               "Partidos",
               "SELECT p.fecha_hora, p.goles, p.asistencias, p.resultado, p.clima, can.nombre "
               "FROM partido p LEFT JOIN cancha can ON p.cancha_id=can.id WHERE p.resultado>0 ORDER BY "
               "p.fecha_hora DESC",
               headers, 6);
}
...
{
    MenuItem items[] = {{1, "Exportar Partidos a XLSX", &menu_exportar_partidos_xlsx_fn},
